      ...
      <v-container>
        <v-form ref="form" v-model="isConfigurationValid">
          ...
          <v-text-field
            ...
            type="number"
            :min="bpmRangeLimits.min"
            :max="bpmRangeLimits.max"
          ...
          <p>
            {{ $t("pages.configure.bpmPrecision")
            }}{{ $t("pages.configure.bpmPrecisionDescription", [bpmPrecisionLimits.min, bpmPrecisionLimits.max]) }}
          </p>
          <v-text-field
            v-model.number="bpmPrecision"
            type="number"
            :min="bpmPrecisionLimits.min"
            :max="bpmPrecisionLimits.max"
          ...
          />
          <v-btn :disabled="!isConfigurationValid || !isConfigurationModified" @click="save" depressed color="primary">{{
            $t("pages.configure.save")
          }}</v-btn>
          <v-btn :disabled="!isConfigurationModified" @click="restore" depressed>{{ $t("pages.configure.restore") }}</v-btn>
        </v-form>
      </v-container>

      <v-card-text
        >{{ $t("info.title")
        }}{{ $t("symbols.parentheses", [$t("pages.configure.about.version") + `${version.major}.${version.minor}.${version.patch}`]) }}</v-card-text
      >
    </v-card>
  </v-container>
</template>
